...
void	ft_editor_gui_init_win_me_1(t_list *head)
{
	...
	ft_gui_elem_init_grid(head, "win_me_sctr", 4, (t_gui_coord){1, 1, 0});
	ft_gui_elem_set_grid(win->child, (t_gui_coord){2, 14, 0}, 10);
	...
	ft_gui_elem_init_grid(head, "win_me_pl", 4, (t_gui_coord){1, 1, 0});
	ft_gui_elem_set_grid(win->child, (t_gui_coord){2, 14, 0}, 10);
...
void	ft_editor_gui_init_win_me(t_list *head)
{
	...
	ft_gui_elem_init_grid(head, "win_me_sprt", 4, (t_gui_coord){1, 1, 0});
	ft_gui_elem_set_grid(win->child, (t_gui_coord){2, 14, 0}, 10);
	ft_gui_elem_set_color(win->child, GUI_CL_SPRITE);
	ft_gui_elem_set_block(win->child);
	ft_gui_elem_set_status(win->child, GUI_ELEM_HIDDEN);
	ft_gui_init_win_setsprite(win->child);
	ft_gui_elem_init_grid(head, "win_me_enemy", 4, (t_gui_coord){1, 1, 0});
	ft_gui_elem_set_grid(win->child, (t_gui_coord){2, 14, 0}, 10);
...
void	ft_gui_init_win_setsector_1(t_list *head)
{
	...
	elem = head->content;
	ft_gui_elem_init_grid(head, "win_me_sctr_title", 1, (t_gui_coord){2, 1, 0});
	ft_gui_elem_set_color(elem->child, 0x000000);
	ft_gui_elem_set_text(elem->child, "Set sector", 16, \
		"fonts/Raleway-Bold.ttf");
	ft_gui_elem_init_grid(head, "win_me_sctr_title", 5, (t_gui_coord){2, 1, 0});
	ft_gui_elem_set_color(elem->child, 0x333333);
	ft_gui_elem_set_text(elem->child, "[Space] -> Save sector", 14, \
		"fonts/Raleway-Bold.ttf");
	ft_gui_elem_init_grid(head, "win_me_sctr_title", 7, (t_gui_coord){2, 1, 0});
	ft_gui_elem_set_color(elem->child, 0x333333);
