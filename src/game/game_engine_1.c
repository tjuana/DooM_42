...
static int	ft_game_engine_pick_sec(t_new_player *pl)
{
    ...
    printf("pick_sec==%d\n",pl->cycle.current->sec_nb);
    if (pl->cycle.current->sec_nb > pl->sectors_nb)
        return (-2);
...
static void put_wall_texture(t_new_player *pl, int wall, int sector, int texture_name)
{
    if (pl->s == wall && pl->cycle.current->sec_nb == sector)//for walls we choose s(wall) and sector
        pl->wall_texture = texture_name;
}

static void put_floor(t_new_player *pl, int sector, int texture_name)
{
    if (pl->cycle.current->sec_nb == sector)//for walls we choose s(wall) and sector
        pl->floor_texture = texture_name;
}

static void put_ceil(t_new_player *pl, int sector, int texture_name)
{
    if (pl->cycle.current->sec_nb == sector)//for walls we choose s(wall) and sector
        pl->ceil_texture = texture_name;
}

static void put_graffiti(t_new_player *pl, int wall, int sector, int texture_name)
{
    if (pl->s == wall && pl->cycle.current->sec_nb == sector)//for walls we choose s(wall) and sector
    {
        pl->wall_graffiti = texture_name;
        pl->graffiti = 1;
    }
}

static void put_skybox(t_new_player *pl, int sector)
{
    if (pl->cycle.current->sec_nb == sector)
    {
        pl->sky_box = 1;
    }
}
...
static int	ft_game_ceil_floor_calcs(t_new_player *pl, int s)
{
	...
	//pl->s = s;
	pl->floor_texture = GREEN;
	pl->ceil_texture = ROCK2;
	pl->wall_texture = ROCK1;
	pl->sky_box = 0;
	pl->graffiti = 0;
    put_wall_texture(pl, 0, 0, FENCE);
    put_wall_texture(pl, 1, 0, WALL);
    put_wall_texture(pl, 3, 0, MARBLE);
    put_ceil(pl, 0, MARBLE);
    put_floor(pl, 0, GRIDBLACK);
    put_graffiti(pl, 1, 0, VENOM);
    put_skybox(pl, 1);
...
static int	ft_game_ceil_floor_calcs_sprite(t_new_player *pl, int s)
{
    int neib;
    ...
    if (ft_game_engine_cross_sprite(pl) == 0)
        return (0);
    pl->ceil.yceil = pl->sect->ceil - pl->pos.z;
    pl->floor.yfloor = pl->sect->floor - pl->pos.z;
    neib = -1;//aaaaaaa pl->sect->neighbors[s];
...
void		ft_game_engine_begin(t_new_player *pl)
{
	...
	{
		if ((pl->s = ft_game_engine_pick_sec(pl)) == -2)
			continue;

		while (++pl->s <= pl->sect->npoints)
		{
			if (ft_game_ceil_floor_calcs(pl, pl->s) == 0)
				continue;
		}
        if (ft_game_ceil_floor_calcs_sprite(pl, pl->s) == 0)
            continue;
		++pl->cycle.rend_sec[pl->cycle.current->sec_nb];
	}
