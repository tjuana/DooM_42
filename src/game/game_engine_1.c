...
/*   Updated: 2020/02/23 17:25:15 by drafe            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_game_engine_scale(t_new_player *pl, int sx1, int sx2)
{
	...
	if (pl->cycle.current->sec_nb == 2 && pl->s == 1)//pl->floor.y2b += 150;
	{
		pl->floor.y2b = WIN_H / 2 - (int)(ft_math_yaw(pl->floor.yfloor + pl->ny1a, \
	pl->t2.y, pl->yaw) * pl->scale_2.y);
	}
		
		
	if (pl->cycle.current->sec_nb == 2 && pl->s == 3)//pl->floor.y1b += 150;
	{
		pl->floor.y1b = WIN_H / 2 - (int)(ft_math_yaw(pl->floor.yfloor + pl->ny1a, \
	pl->t1.y, pl->yaw) * pl->scale_1.y);
	}
			
		
	if (pl->cycle.current->sec_nb == 2 && pl->s == 2)//pl->floor.y1b += 150;//pl->floor.y2b += 150;
	{
		pl->floor.y1b = WIN_H / 2 - (int)(ft_math_yaw(pl->floor.yfloor + pl->ny1a, \
	pl->t1.y, pl->yaw) * pl->scale_1.y);
		pl->floor.y2b = WIN_H / 2 - (int)(ft_math_yaw(pl->floor.yfloor + pl->ny1a, \
	...
	if (pl->cycle.current->sec_nb == 2 && pl->s == 1)
	{
		//pl->floor.y2b += 150;
	}
	if (pl->cycle.current->sec_nb == 2 && pl->s == 3)
	{
		//pl->floor.y1b += 150;
		
	}
	if (pl->cycle.current->sec_nb == 2 && pl->s == 2)
	{
/*		pl->ceil.y1a += pl->y1a;
		pl->ceil.y2a += pl->y2a;
		pl->ceil.ny1a += pl->ny1a;
		pl->ceil.ny2a += pl->ny2a;
		
		pl->floor.y1b += pl->y1b;
		pl->floor.y2b += pl->y2b;
		pl->floor.ny1b += pl->ny1b;
		pl->floor.ny2b += pl->ny2b;
*/
		//pl->floor.y1b += 150;
		//pl->floor.y2b += 150;
	}
