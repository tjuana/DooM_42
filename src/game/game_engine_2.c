...
void			ft_game_draw_ceil_floor(int x, t_new_player *pl)
{
	...
	while (++pl->t.y <= pl->y_bot[x])
	{
		...
		if (pl->t.y < pl->ceil.cya && pl->sky_box == 0)
			ft_game_pix1(pl, pl->ceil_texture);
		if (pl->t.y < pl->ceil.cya && pl->sky_box == 1)
			ft_game_pix_sky(&pl->t, pl);
		if (pl->t.y >= pl->ceil.cya)
			ft_game_pix1(pl, pl->floor_texture);
	}
}
...
static void		ft_game_engine_ceil_floor(t_new_player *pl, int x, int neib)
{
	...
	if (neib >= 0)
	{
		...
		ft_game_draw_walls(x, pl, WALL_TOP, pl->wall_texture);
		pl->y_top[x] = ft_math_clamp(ft_math_max(pl->ceil.cya, pl->ceil.cnya),\
		pl->y_top[x], WIN_H - 1);
		ft_game_draw_walls(x, pl, WALL_BOTT, pl->wall_texture);
	...
	{
	    printf("%d\n", pl->wall_texture);
		ft_game_draw_walls(x, pl, WALL_FULL, pl->wall_texture);
		if (pl->graffiti == 1)
		    ft_game_draw_graffiti(x, pl, WALL_FULL, pl->wall_graffiti);
	}
}
