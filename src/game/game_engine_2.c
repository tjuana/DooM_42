...
/*   Updated: 2020/02/23 19:25:40 by drafe            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_game_engine_ceil_floor(t_new_player *pl, int x, int neib)
{
	...
	if (neib >= 0)
	{
		...
		pl->y_top[x] = ft_math_clamp(ft_math_max(pl->ceil.cya, \
		pl->ceil.cnya),	pl->y_top[x], WIN_H - 1);

		ft_game_draw_walls(x, pl, WALL_BOTT, pl->n);
		pl->y_bot[x] = ft_math_clamp(ft_math_min(pl->ceil.cyb, \
		pl->ceil.cnyb), 0, pl->y_bot[x]);
	}
	else
	{
