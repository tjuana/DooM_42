...
/*   Updated: 2020/02/07 16:32:48 by dorange-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				int size, int color)
{
	...
	ft_gui_set_sdl_color(&w->sdl->font.color, color);
	...
	while (fonts_list)
	{
		...
		if (ft_strcmp(font_path, font->path) == 0 && size == font->f_sz)
		{
			...
			if (TTF_GlyphMetrics(w->sdl->font.ptr, 'A', 0, 0, 0, 0, \
				&w->sdl->font.g_sz) == -1)
				...
				char *str, t_gui_coord c)
{
	...
	if ((c.x < 0) || (c.y < 0) || (str == NULL) || \
		...
		return ;
	tmp_texture = NULL;
	if (TTF_SizeText(w->sdl->font.ptr, str, \
		&w->sdl->font.w, &w->sdl->font.h) == -1)
		ft_error("FONT ERROR (4)");
	tmp_rect = ft_gui_create_sdl_rect(w->sdl->font.w, \
		w->sdl->font.h, c.x, c.y);
	if (!(text_surf = TTF_RenderText_Blended(w->sdl->font.ptr, \
		str, w->sdl->font.color)))
		ft_error("FONT ERROR (5)");
	else
	{
		w->sdl->renderer == NULL ? ft_putstr_fd(SDL_GetError(), 2) : 0;
		tmp_texture = SDL_CreateTextureFromSurface(w->sdl->renderer, \
			text_surf);
		tmp_texture == NULL ? ft_putstr_fd(SDL_GetError(), 2) : 0;
		tmp_texture == NULL ? ft_error("FONT ERROR (6)") : 0;
		SDL_FreeSurface(text_surf);
	}
	if (SDL_RenderCopy(w->sdl->renderer, tmp_texture, 0, \
		tmp_rect) != 0)
